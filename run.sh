...
INPUT_IMAGE="${1:-input_output/input.png}"
...
# Absolute project directory: folder containing this run.sh
...
if ! $DOCKER_CMD image inspect cpp-blur-worker >/dev/null 2>&1; then
    echo "Docker worker image not found. Building once..."
    ...
    echo "Docker worker image already exists. Skipping build."
fi
...
for padding in "${PADDING_MODES[@]}"; do
    for filter_size in "${FILTER_SIZES[@]}"; do
        for workers in "${WORKERS_LIST[@]}"; do
            ...
            # Launch with workers + 1 MPI processes:
